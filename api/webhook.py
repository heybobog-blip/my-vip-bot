...
# =========================================================
# ฟังก์ชันแกะซอง (ฉบับแก้: ทะลุ Error 400 ถ้าเงินเข้าจริง)
# =========================================================
def redeem_truemoney(url, phone_number):
    try:
        ...
        # [ส่วนที่แก้ไข] พยายามแกะข้อมูล JSON ก่อนเสมอ ไม่สน HTTP Code
        try:
            data = response.json()
        except json.JSONDecodeError:
            # ถ้าแกะไม่ออก ค่อยไปเช็ค HTTP Code
            if response.status_code != 200:
                return {"status": "error", "message": f"Server Error ({response.status_code})"}
            return {"status": "error", "message": "Invalid Response"}

        # ตรวจสอบสถานะจากไส้ใน JSON (สำคัญที่สุด)
...
async def handle_truemoney(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # เรียกฟังก์ชันรับเงิน (แบบ Non-blocking)
